...
def get_cat_breeds():
   ...
   headers = {"x-api-key": THE_CAT_API_KEY} # Заголовок для запроса
   response = requests.get(url, headers=headers) # Запрос к API
   return response.json() # Возвращение ответа в виде списка json

# Функция для получения картинки кошки по породе
def get_cat_image_by_breed(breed_id): # функция
   url = f"https://api.thecatapi.com/v1/images/search?breed_ids={breed_id}" # f строка для того, чтобы добавлять своё
   headers = {"x-api-key": THE_CAT_API_KEY} # Заголовок для запроса
   response = requests.get(url, headers=headers) # Запрос к API
   data = response.json() # Возвращение ответа в виде списка json в data виде словаря
   return data[0]['url'] # Возвращение фото из словаря data
